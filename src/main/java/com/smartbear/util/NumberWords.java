...
public final class NumberWords {
    ...
    public static String hourWord(int h12) { // 1..12
        return switch (h12) {
    ...
    public static String minuteWord(int m) { // 0..59, no “oh” prefix; spaces not hyphens
        if (m < 20) return ONES[m];
        int tens = m / 10, ones = m % 10;
        return ones == 0 ? TENS[tens] : TENS[tens] + " " + ONES[ones];
    }

    public static String minuteWordDigital(int m) { // add natural “oh” for 01..09
